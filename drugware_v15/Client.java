...
public void ajoutPrescriptions(Prescription prescription) {
	this.prescriptions.add(prescription);
}
public boolean siPrescriptionExiste(Client cl , String medicament) {
	
	for (Iterator<Prescription> it = this.prescriptions.iterator(); it.hasNext();) {
		Prescription itPrec = it.next();
		if (itPrec.getMedicamentAPrendre().equals(medicament)) {
			return true;
		}
	}
	return false;
